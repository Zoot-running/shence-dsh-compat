...
// src/outage-guard.ts
import { appendFileSync as appendFileSync2, existsSync as existsSync2, mkdirSync as mkdirSync2, readFileSync as readFileSync2 } from "node:fs";
import { join as join2 } from "node:path";
var bursts = /* @__PURE__ */ new Map();
var OUTAGE_BURST_N = 3;
var OUTAGE_BURST_MS = 12e4;
function observeProviderFailure(provider, now = Date.now()) {
...
function observeProviderSuccess(provider, now = Date.now()) {
  const b = bursts.get(provider);
  if (b === void 0) return;
  bursts.set(provider, { consecutive: 0, start: now, lastAt: now, open: false });
}
function recordOutage(provider, from, to) {
  try {
    const dir = join2(process.env.DSH_HOME ?? ".", "storages");
    mkdirSync2(dir, { recursive: true });
    appendFileSync2(
      join2(dir, "provider-outages.jsonl"),
...
function inOutageWindow(at, windows) {
